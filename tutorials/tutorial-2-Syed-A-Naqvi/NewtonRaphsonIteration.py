...
# order of arguments:  function f(x), derivative, initial guess, max iterations, tolerance on approximate error, tolerance on residual
    ...
    for k in range(kMax):
        ...
        if(k>0):                                                   # ensuring that the iteration history recording process begins AFTER the first iteration as the
            iter_Hist = np.vstack((iter_Hist,current_Iter))        # inital iteration is comprised of error values for the initial guess and not for the first update
